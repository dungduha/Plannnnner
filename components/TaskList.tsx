...
import { GripVertical, Check, Trash2, ArrowRightCircle, ArrowLeftCircle, Clock, X, ChevronDown, Calendar as CalendarIcon, ChevronLeft, ChevronRight } from 'lucide-react';
...
interface TaskListProps {
    ...
    onUpdateText: (id: number, t: string) => void;
    onUpdateTime: (id: number, t: string | undefined) => void;
    onCycleCategory: (id: number) => void;
    onCycleType: (id: number) => void;
    onUpdateWeeklyDay: (id: number, d: number) => void;
    onUpdateNotes: (id: number, n: string) => void;
    onUpdateCategory: (id: number, c: Category) => void;
    onUpdateType: (id: number, t: TaskType) => void;
...
// --- Helper Hook for Popover Positioning ---
const usePopoverPosition = (triggerRef: React.RefObject<HTMLElement>, isOpen: boolean) => {
    ...
    useLayoutEffect(() => {
        if (isOpen && triggerRef.current) {
            ...
            // Basic viewport boundary check (prevent going off screen right)
            if (left + 240 > window.innerWidth) {
                left = window.innerWidth - 250; // shift left
            }
...
export const DatePickerPopover: React.FC<{
...
}> = ({ currentDate, onSelect, onClose, triggerRef }) => {
            ...
            days.push(
                <button
                    ...
                    className={`aspect-square flex items-center justify-center rounded-lg text-xs font-bold transition-all
                        ${isSelected ? 'bg-indigo-600 text-white shadow-md' : 'hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300'}
    ...
    return (
        <div 
            ref={popoverRef}
            style={style}
            className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border dark:border-slate-700 p-4 w-64 animate-fade-in-up"
        >
...
export const TimePickerPopover: React.FC<{ 
    ...
    onClose: () => void,
    triggerRef: React.RefObject<HTMLElement>
}> = ({ currentTime, onSave, onClose, triggerRef }) => {
    ...
    return (
        <div 
            ...
            className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border dark:border-slate-700 p-4 w-60 animate-fade-in-up cursor-auto"
        >
            ...
            <div className="flex gap-2 items-center justify-center">
                <div ref={hourRef} className="flex flex-col gap-1 w-full">
                    <select 
                ...
                <div ref={minuteRef} className="flex flex-col gap-1 w-full">
                    <select 
...
export const EditTaskModal: React.FC<{ 
    ...
    onClose: () => void, 
    onDelete: (id: number) => void 
}> = ({ task, onChange, onClose, onDelete }) => {
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-md bg-white dark:bg-slate-900 rounded-[2rem] p-6 shadow-2xl animate-fade-in-up flex flex-col gap-4 max-h-[90vh] overflow-y-auto no-scrollbar">
                ...
                <input 
                    ...
                    autoFocus
                    className="text-xl font-bold bg-transparent border-b-2 border-slate-100 dark:border-slate-800 pb-2 outline-none focus:border-indigo-500 dark:text-white transition-colors shrink-0"
                    placeholder="Task name"
                />

                {/* Flex Row: Category | Date | Time */}
                <div className="flex gap-2 shrink-0">
                    ...
                    <div className="flex-1 min-w-[30%]">
                        ...
                        <div className="relative">
                            <select 
                                ...
                                className="w-full h-11 bg-slate-100 dark:bg-slate-800 rounded-xl px-2 text-sm font-bold appearance-none outline-none text-slate-700 dark:text-slate-200 border-r-[16px] border-transparent cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
                            >
                    ...
                    <div className="flex-1 min-w-[30%]">
                        ...
                        <div 
                            ...
                            className={`relative h-11 bg-slate-100 dark:bg-slate-800 rounded-xl flex items-center justify-center transition-colors hover:bg-slate-200 dark:hover:bg-slate-700 cursor-pointer ${activePopover === 'date' ? 'ring-2 ring-indigo-500/20' : ''}`}
                        >
                            <div className="flex items-center gap-1.5 pointer-events-none text-slate-900 dark:text-white">
                                ...
                                <span className="text-xs font-bold pt-0.5 truncate">
                                    {new Date(task.dateCreated).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                                </span>
                            </div>
                        </div>
                        {/* Render outside normal flow via portal logic (handled by fixed positioning in component) */}
                        {activePopover === 'date' && (
                            <DatePickerPopover 
                                currentDate={task.dateCreated}
                                onSelect={(d) => { updateField('dateCreated', d); setActivePopover('none'); }}
                    ...
                    <div className="flex-1 min-w-[30%]">
                        ...
                        <div 
                            ...
                            className={`relative h-11 bg-slate-100 dark:bg-slate-800 rounded-xl flex items-center justify-center transition-colors hover:bg-slate-200 dark:hover:bg-slate-700 cursor-pointer ${activePopover === 'time' ? 'ring-2 ring-indigo-500/20' : ''}`}
                        >
                            {/* Centered Content */}
                            ...
                            {/* Absolute Close Button - Does not affect centering */}
                            {task.time && (
                                <button 
                                    onClick={(e) => {
                                        e.preventDefault();
                                    ...
                                    className="absolute top-1/2 -translate-y-1/2 right-2 bg-red-100 text-red-500 dark:bg-red-900/30 rounded-full p-0.5 shadow-sm z-30 hover:scale-110 transition-transform cursor-pointer"
                                >
                        ...
                        {activePopover === 'time' && (
                            <TimePickerPopover 
                                ...
                                onSave={(t) => { updateField('time', t); setActivePopover('none'); }}
                                onClose={() => setActivePopover('none')}
                     ...
                     <div className="flex gap-2">
                        {(['one-time', 'recurring', 'weekly'] as TaskType[]).map(t => (
                            <button 
                                ...
                                className={`flex-1 h-11 rounded-xl text-[10px] font-black uppercase transition-all flex items-center justify-center ${
                                    task.type === t
                     ...
                     {task.type === 'weekly' && (
                        <div className="mt-3 bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl flex items-center justify-between animate-fade-in">
                             ...
                             <select 
                                ...
                                onChange={(e) => updateField('weeklyDay', parseInt(e.target.value))}
                ...
                <div className="flex-grow min-h-0">
                    ...
                    <textarea 
                        ...
                        className="w-full h-full min-h-[100px] bg-slate-50 dark:bg-slate-800 rounded-xl p-3 text-sm font-medium outline-none resize-none dark:text-slate-300 focus:ring-2 focus:ring-indigo-100 dark:focus:ring-indigo-900/30 transition-all"
                    />
                </div>
    ...
    return (
        ...
        >
            <div className="flex items-center w-full">
                <div className={`mr-4 transition-colors px-1 touch-none drag-handle cursor-grab active:cursor-grabbing text-slate-200 dark:text-slate-600 group-hover:text-slate-400`}>
                        <GripVertical size={20} />
                </div>
                
                <button 
                    onClick={() => onToggle(task.id)}
                    className={`w-8 h-8 rounded-full border-2 flex items-center justify-center transition-all shrink-0 ${isDone ? 'bg-indigo-600 border-indigo-600 text-white' : 'border-slate-200 dark:border-slate-600 hover:border-indigo-400'}`}
                >
                ...
                >
                    <div className="flex items-center gap-2 flex-wrap">
                        {hasTime && (
                            <span className="text-[10px] font-black bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 px-2 py-0.5 rounded-md border border-indigo-100 dark:border-indigo-800/50">
                                {task.time}
                            </span>
                        )}

                        <span className={`text-base font-bold tracking-tight truncate block min-w-[50px] ${isDone ? 'line-through text-slate-400' : 'text-slate-800 dark:text-slate-200'}`}>
                            {task.text}
                        </span>
...
const TaskList: React.FC<TaskListProps> = ({ 
    ...
    return (
        <div ref={listRef} className="space-y-4 pb-32 animate-fade-in">
            {tasks.map(task => (
                <TaskItem 
                    key={task.id} 
                    task={task} 
